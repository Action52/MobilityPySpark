        ...
        if depth == max_depth:
            ...
            rdd = instants_at.rdd.mapPartitionsWithIndex(
                lambda i, x: KDTreePartitionSpark.partition_median_index(
                    i, x, dim=dim, rowcol="rowNo"
                )
            )
            medians = [median for median in rdd.toLocalIterator() if median[1]]
            ...
            if dim == "ts":
                dim = "t"
            bboxleft = STBoxWrap(
                new_bounds_from_axis(bounds, dim, midrow, "left").__str__()
            )
            bboxright = STBoxWrap(
                new_bounds_from_axis(bounds, dim, midrow, "right").__str__()
            )

            left = KDTreePartitionSpark._generate_grid(
                spark,
                lower,
                instantstablename,
                traj_colname,
            ...
            right = KDTreePartitionSpark._generate_grid(
                spark,
                upper,
                instantstablename,
                traj_colname,
