...
class ApproximateAdaptiveBinsPartitioner(MobilityPartitioner):
    ...
    def __init__(
        ...
        self.grid = self.spark_generate_grid(
            df, colname, bounds, num_tiles, dimensions, utc
        )
    ...
    def spark_generate_grid(
        ...
        while unchecked_dims:
            ...
            reduced_bounds = (
                ...
                .reduceByKey(
                    lambda x, y: ApproximateAdaptiveBinsPartitioner.reduce_generate_grid_dim(
                        x, y
                    )
                )
                .collect()
            ...
            new_tiles[cur_dim] = (
                ApproximateAdaptiveBinsPartitioner.make_bounds_contiguous(
    ...
    def map_generate_grid_dim(
        movingobjects, colname, bounds: STBox, dim: str, num_tiles: int, utc: str
    ):
        pymeos_initialize(utc)
        ...
        return [
            (i, (bin[0], bin[1])) for i, bin in enumerate(binvals) if bin[0] != bin[1]
        ]

    @staticmethod
    def reduce_generate_grid_dim(val1, val2):
    ...
    @staticmethod
    def map_generate_stbox_dim(key, vals, utc, dim, bounds, num_tiles):
        pymeos_initialize(utc)
        minval = vals[0]
        maxval = vals[1]
        if key == 0:
    ...
    def make_bounds_contiguous(bounds_list, real_bounds, dim):
        ...
        for i, (index, (min_bound, max_bound)) in enumerate(sorted_bounds):
            ...
            contiguous_bounds.append((i, (adjusted_min_bound, adjusted_max_bound)))
        ...
        for key, (min_bound, max_bound) in contiguous_bounds:
            ...
            box = STBoxWrap(new_bounds(real_bounds, dim, minval, maxval).__str__())
...
def main():
    ...
    mdtpart = ApproximateAdaptiveBinsPartitioner(
        bounds=bounds, num_tiles=3, dimensions=["x", "y"], utc="UTC"
    )
